...
export default function DashboardLayout() {
  ...
  // Auto-expand Staff sub-menu when on any /staff route
  useEffect(() => {
                ...
                <div className="flex items-center gap-1">
                  <NavLink to={item.to}
                    ...
                    className={`flex flex-1 items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors ${
                      isStaffActive() ? "bg-sidebar-accent text-sidebar-accent-foreground"
                                      : "text-sidebar-foreground hover:bg-sidebar-accent/50"
                    }`}>
                    {item.icon}{item.label}
                  </NavLink>
                  <button
                    ...
                    className={`rounded-md p-1.5 transition-colors hover:bg-sidebar-accent/50 ${
                      isStaffActive() ? "text-sidebar-accent-foreground" : "text-sidebar-foreground"
                    }`}>
                    <ChevronDown className={`h-3.5 w-3.5 transition-transform ${staffExpanded ? "rotate-180" : ""}`} />
                  </button>
                </div>
