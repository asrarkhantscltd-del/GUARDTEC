...
// content already rendered elsewhere in this app.
export function InstructionStep({ step }: { step: AckStep }) {
  return (
    ...
    >
      {step.html ? (
        <div className="prose prose-sm max-w-none text-sm leading-relaxed text-foreground" dangerouslySetInnerHTML={{ __html: step.html }} />
      ) : (
